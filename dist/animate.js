class animate {
  ...
  setAllScreens() {
      ...
      entries.forEach(entry => {
        ...
        if (entry.target.dataset.threshold) {
          if (entry.isIntersecting === true && entry.target.dataset.threshold <= Math.floor(entry.intersectionRatio * 100)) {
            ...
            console.log(entry.intersectionRatio * 100); // alert(1);
    ...
    this.screens.forEach(screen => {
      ...
      this.observer = new IntersectionObserver(callback, options); // let target = document.querySelector('.sec8');
    ...
    if (this.DEBUG) {
      ...
      this.screens.forEach(screen => {
        screen.style.position = 'relative'; // screen.style.position = 'opacity';

        const threshold = document.createElement('span');
        threshold.classList.add('threshold');
        threshold.textContent = 0;
        threshold.setAttribute('title', '% part of the element is visible');
  ...
  animateSection(screen) {
    ...
    const elements = screen.querySelectorAll('[data-animations]'); // console.log(elements);
